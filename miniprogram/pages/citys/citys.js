...
Page({
  ...
  */
  onLoad: function (options) {
    console.log(options)
    if ((options.city==("城市"))==false){
    this.setData({
      nowcity: options.city
    })
    }
  ...
  */
  ...
  citySelected: function (e) {
    ...
    var city= pages[pages.length-2]

    var cityNameTemp = e.currentTarget.dataset.cityname

    
      city.setData({city: cityNameTemp})
      this.setData({
        nowcity: cityNameTemp,
      })    
  ...
  scorallreset: function () {
    ...
    query.select('.search-container').boundingClientRect(function (res) {
      ...
      t1hight = res.bottom;
      console.log('search-container', t1hight)
      console.log('search-container', res)
      console.log('search-container', t1hight, windowHeight, (windowHeight - t1hight))
      that.setData({
        scrollheight: (windowHeight - t1hight),
      })
    }).exec()
    //在这里做计算，res里有需要的数据
 ...
 */
  data: {
    ...
    hotcitys: [{ cityName: '热门', cityPinYin: '', cityPY: '' }, { cityName: '上海', cityPinYin: '##', cityPY: '##' }, { cityName: '北京', cityPinYin: '##', cityPY: '##' }, { cityName: '广州', cityPinYin: '##', cityPY: '##' }, { cityName: '深圳', cityPinYin: '##', cityPY: '##' }, { cityName: '杭州', cityPinYin: '##', cityPY: '##' }],
